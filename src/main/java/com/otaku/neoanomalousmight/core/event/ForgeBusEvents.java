    ...
    public static void onPlayerAttack(LivingAttackEvent event) {
        ...
        if (attacker != null && attacker instanceof Player player) {
            ...
            player.getCapability(PlayerRoleProvider.PLAYER_ROLE_CAPABILITY).ifPresent(role -> {
                if (role.hasSelectedRole()) {
                    ElementalInteraction.applyElementalEffect(player, target, role.getSelectedRole().getElementType(), com.otaku.neoanomalousmight.element.ElementType.NONE);
                }
            });
        }
    }
        ...
        if (entity instanceof Player player) {
            ...
            player.getCapability(PlayerRoleProvider.PLAYER_ROLE_CAPABILITY).ifPresent(role -> {
                if (role.hasSelectedRole()) {
                    ...
                    float defense = role.getDefense();
                    float reducedDamage = damage * (1 - Math.min(0.8f, defense / (defense + 100)));

                    event.setAmount(reducedDamage);

                    // 检查闪避
                    float evasion = role.getEvasion();
                    if (player.level().random.nextFloat() < evasion) {
                    ...
                    if (source.getEntity() instanceof LivingEntity attacker) {
                        attacker.getCapability(PlayerRoleProvider.PLAYER_ROLE_CAPABILITY).ifPresent(attackerRole -> {
                            if (attackerRole.hasSelectedRole()) {
                                float critChance = attackerRole.getCritChance();
                                if (player.level().random.nextFloat() < critChance) {
                                    float critDamage = attackerRole.getCritDamage();
                                    event.setAmount(reducedDamage * critDamage);
                                }
                            }
                        });
                    }
        ...
        public static void onClientPlayerTick(TickEvent.ClientTickEvent event) {
            if (event.phase == TickEvent.Phase.END) {
                ...
                if (player != null) {
                    ...
                    player.getCapability(PlayerRoleProvider.PLAYER_ROLE_CAPABILITY).ifPresent(role -> {
                        if (role.hasSelectedRole()) {
                            float movementSpeed = role.getMovementSpeed();
                            player.setSpeed((float) (player.getSpeed() * movementSpeed));
                        }
                    });
                }
            }
        ...
        public static void onServerPlayerTick(TickEvent.PlayerTickEvent event) {
            if (event.phase == TickEvent.Phase.END) {
                ...
                if (!player.level().isClientSide()) {
                    ...
                    player.getCapability(PlayerRoleProvider.PLAYER_ROLE_CAPABILITY).ifPresent(role -> {
                        if (role.hasSelectedRole()) {
                            float maxHealth = role.getMaxHealth();
                            if (player.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.MAX_HEALTH).getValue() != maxHealth) {
                                // 使用属性系统来修改最大生命值
                                ...
                                if (healthAttribute != null) {
                                    // 移除旧的修改器
                                    healthAttribute.removeModifier(com.otaku.neoanomalousmight.core.util.HealthAttributeModifier.HEALTH_MODIFIER_ID);

                                    // 添加新的修改器
                                    healthAttribute.addTransientModifier(new net.minecraft.world.entity.ai.attributes.AttributeModifier(
                                            com.otaku.neoanomalousmight.core.util.HealthAttributeModifier.HEALTH_MODIFIER_ID,
                                            "Role Health Modifier",
                                            maxHealth - player.getMaxHealth(),
                                            net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADDITION
                                    ));
                                    

                                    // 如果玩家当前生命值超过新的最大生命值，将其设置为最大生命值
